              ...
              {activeSidebarItems.map((item, index) => {
                return (
                  <Link
                    key={item.id}
                    href={item.url}
                    className={`hover:bg-slatedark group relative flex items-center gap-2.5 rounded-sm px-4 py-2 font-medium text-bodydark1 duration-300 ease-in-out dark:hover:bg-meta-4 ${pathname == item.url && "bg-slatedark dark:bg-meta-4"
                      }`}
                  >
                    <div
